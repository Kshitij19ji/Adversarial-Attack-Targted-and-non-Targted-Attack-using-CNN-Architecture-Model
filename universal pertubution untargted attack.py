...
def untargeted_uap_attack(model, dataloader, epsilon=0.02, alpha=0.005, num_iter=20, device='cuda'):
    model.eval()
...
def plot_untargeted_uap(X_sel_norm, v, y_true, y_pred, title="Untargeted UAP", M=2, N=5):
    fig, ax = plt.subplots(M, N, figsize=(12, 6))
    for i in range(M):
        for j in range(N):
            k = i * N + j
            if k >= X_sel_norm.size(0):
                break
            ...
            t, p = y_true[k].item(), y_pred[k].item()
...
success = (preds_adv != y_sel.to(device)).float().mean().item()
print(f" Untargeted UAP Success Rate (VGG19): {success * 100:.1f}%")
plot_untargeted_uap(X_sel, v, y_sel.cpu(), preds_adv.cpu(), title=f"Untargeted UAP (True Label {true_lbl})")


orig_dir = "/data1/kshitij/PGD/UNIVERSAL UT/universalOriginalimage"
attack_dir = "/data1/kshitij/PGD/UNIVERSAL UT/universalAttackimage"
...
for i in range(k_imgs):
    ...
    TF.to_pil_image(orig_img).save(os.path.join(orig_dir, f"universal{index}.png"))
    TF.to_pil_image(adv_img).save(os.path.join(attack_dir, f"universalAttack{index}.png"))
